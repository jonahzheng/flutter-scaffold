...
import 'package:MoFangXiu/common/net/NetCode.dart';

import 'dart:collection';

import 'package:MoFangXiu/common/config/Config.dart';
import 'package:MoFangXiu/common/utils/LocalStorage.dart';
...
class HttpManager {
  ...
  static netFetch(url, params, Map<String, String> header, Options option, {noTip = false}) async {
    ...
    if (connectivityResult == ConnectivityResult.none) {
      return new NetData(NetCode.NETWORK_ERROR, NetCode.errorHandleFunction(NetCode.NETWORK_ERROR, "", noTip));
    }
    ...
    try {
      ...
      return new NetData(errorResponse.statusCode, NetCode.errorHandleFunction(errorResponse.statusCode, e.message, noTip));
    }
    ...
    try {
      if (option.contentType != null && option.contentType.primaryType == "text") {
        return new NetData(NetCode.SUCCESS,response.data);
      } else {
      ...
      return new NetData(response.statusCode, response.data);
    }
    return new NetData(response.statusCode, NetCode.errorHandleFunction(response.statusCode, "", noTip));
  }
